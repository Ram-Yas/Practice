...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i < answers.length; i++) { //her bir ogrenciye bakar ogrenci -->i
            int count = 0;// her bir ogrenci icin dogru cevaplari sayacagi icin, sayac buraya konur
            for (int j = 0; j < answers[i].length; j++) { //her bir ogrencinin cevaplarina bakar  ogrenci cevaplari -->

                if (answers[i][j] == keys[j]) { //cevap anahtari ile ogrenci cevaplarini karsilastiriyor  -- cevaplar-->j oldugu icin keys[j]kullandik
                    count++; //
                }

            }
            System.out.println(i + 1 + " no'lu ogrencinin dogru cevap sayisi : " + count);
        }
    }
